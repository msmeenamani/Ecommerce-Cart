...
describe("Test e-commerce Test C", () => {
  ...
  it('post the item data', async done => {
    await request.post('/ecom/item').send(itemArray[0]).then(async e => {
      ...
      await request.post('/ecom/item').send(itemArray[1]).then(async item => {
        ...
        await request.post('/ecom/item').send(itemArray[2]).then(async res => {
            itemIds.push(res.body.data[0]._id)
            await request.post('/ecom/item').send(itemArray[3]).then(async res1 => {
              itemIds.push(res1.body.data[0]._id)
  ...
  it('checkout item', async done => {
    ...
    expect(res.body.statusCode).toBe(200)
    expect(res.body.data.total).toBe(155)
    done()
  })
